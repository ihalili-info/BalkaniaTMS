...
* **Access Control:** Authentication via **Supabase Auth**; `profiles.role` decides which modules are reachable. `admin` gets everything including Integrations and its connector configuration; `dispatcher` gets everything else. Enforced in the database by RLS (migration 0004), not only in the UI — the sidebar filter is a convenience, the policies are the control. New accounts default to `dispatcher`, and nobody can edit their own role. Each user has an **Account** page (`/account`) to change their own name and password — the password change verifies the current one first — but role, email and depot are admin-managed.
...
-- Cache of traffic-aware HERE truck-routed legs (truck -> its next undelivered
-- stop), shared across serverless instances. A pure cache: a miss costs a
-- routing call, never a wrong answer, and it may be truncated at any time.
